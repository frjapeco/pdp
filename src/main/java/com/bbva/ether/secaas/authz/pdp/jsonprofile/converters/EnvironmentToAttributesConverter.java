...
import com.bbva.ether.secaas.authz.pdp.jsonprofile.model.Environment;
...
@Component
public class EnvironmentToAttributesConverter extends CustomConverter<Environment, Attributes> {

    @Autowired
    private OrikaBeanMapper mapper;

    public Attributes convert(Environment source, Type<? extends Attributes> destinationType, MappingContext ctx) {
        String id = source.getId();
        URI categoryId = URI.create(source.getCategoryId());
        Element content;
        content = Optional.ofNullable(source.getContent())
